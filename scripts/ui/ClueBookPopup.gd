...
func open_ui(entries: Array[Dictionary]) -> void:
	_rebuild(entries)
	_overlay.show()
...
func _build_shell() -> void:
	...
	var hint := Label.new()
	hint.text = "记录在调查中发现的资料与指引。点击“查看”可重新阅读。"
	hint.add_theme_font_size_override("font_size", 14)
	hint.add_theme_color_override("font_color", Color(0.78, 0.75, 0.66, 1.0))
	box.add_child(hint)
...
func _build_row(entry: Dictionary) -> Control:
	...
	if texture != null:
		...
		placeholder.text = "教程" if String(entry.get("entry_type", "")) == "text_pages" else "资料"
